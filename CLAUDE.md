...
The Venues section starts with an overview of every imported venue with a summary of its activities (`VenuesOverview`). Opening a venue shows its activities (`#venues/<id>`, `ActivityManagerCard`), with a breadcrumb back to the overview, and the venue's widget setting for unavailable activities (show them disabled with DMN's reason, or hide them; the `dmn_hide_unavailable` venue meta, applied by `GET dmn/v1/booking-types`), which saves as soon as it changes. The activity editor stays mounted behind the overview, so going back keeps unsaved edits (the venue's card shows them), and opening a different venue asks before discarding them (`VenuesPanel`).
